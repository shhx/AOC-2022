...
def get_priority(state: State):
    ...
    robots = state.rb_geo * 1000 + state.rb_obs * 100 + state.rb_cy * 10 + state.rb_ore
...
for blueprint in blueprints:
    ...
    while to_visit:
        ...
        # print(old_state)
        if max_geodes > get_max_geodes(old_state):
            continue
        if len(to_visit) >= prune*1.2:
            # print("Pruning")
            rem = list(to_visit)
        ...
        if len(visited) % 10000 == 0:
            print(state, len(to_visit), get_max_geodes(old_state))
    
    print("Best: ", best_state)
    print("Max: ", max_geodes)
    out.append(max_geodes * blueprint.id)
